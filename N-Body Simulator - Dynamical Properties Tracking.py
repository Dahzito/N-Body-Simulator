...
def Visualize(energy_reset_period=Δt_reset):
    ...
    try:
        fig.canvas.manager.set_window_title("N-Body Simulation - Dynamical Properties Tracking")
    except Exception:
        pass
    save_png_template = os.path.join(
        ...
        "N-Body Simulator, Dynamical Properties, HD 28109 System",
    ...
    save_json_template = os.path.join(
        ...
        "N-Body Simulator - Dynamical Properties Tracking",
        "N-Body Simulator, Dynamical Properties, HD 28109 System",
        "N-Body Simulator, HD 28109 System Data {num} @ {current_time:.2f} days.json"
    )
    ...
    gs = fig.add_gridspec(3, 2, height_ratios=[1, 1, 1], width_ratios=[1, 1], hspace=0.4, wspace=0.4)
    ax_energy = fig.add_subplot(gs[0, 0])  # Energy plot
    ax_force = fig.add_subplot(gs[0, 1])   # Force plot
    ax_mass = fig.add_subplot(gs[1, 0])    # Mass plot
    ax_flux = fig.add_subplot(gs[1, 1])    # Flux plot
    ax_temperature = fig.add_subplot(gs[2, 0])  # Temperature plot
    ...
    # Create an external legend for all bodies
    fig.subplots_adjust(right=0.80)
    body_handles = [Line2D([0], [0], color=b.rgb, lw=2) for b in bodies]
    ...
    ax_energy.set_title("Total Mechanical Energy", fontsize=10, fontweight='bold', color='#e0e0e0')
    ...
    ax_force.set_title("Total Gravitational Force on Each Body", fontsize=10, fontweight='bold', color='#e0e0e0')
    ...
    ax_mass.set_title("Mass of Each Body", fontsize=10, fontweight='bold', color='#e0e0e0')
    ...
    ax_flux.set_title("Flux Received by Each Body", fontsize=10, fontweight='bold', color='#e0e0e0')
    ax_flux.set_xlabel("Time (days)", fontsize=9)
    #ax_flux.legend()
    ...
    ax_temperature.set_title("Temperature of Each Body", fontsize=10, fontweight='bold', color='#e0e0e0')
    ...
    def init():
        return energy_lines + archived_energy_lines + force_lines + archived_force_lines + mass_lines + archived_mass_lines + flux_lines + archived_flux_lines + temperature_lines + archived_temperature_lines + legend_artists
    ...
    def update(frame):
        ...
        if temperature_data and any(temperature_data):
            ...
            ax_temperature.set_ylim(min_temp * 0.9, max_temp * 1.1)
        """    
        if temperature_data and any(temperature_data):
